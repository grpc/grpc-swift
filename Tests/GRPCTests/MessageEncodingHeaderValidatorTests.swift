...
class MessageEncodingHeaderValidatorTests: GRPCTestCase {
  ...
  func testSupportedAlgorithm() throws {
    let validator = MessageEncodingHeaderValidator(
      encoding: .enabled(.init(enabledAlgorithms: [.deflate, .gzip], decompressionLimit: .absolute(10)))
    )
  ...
  func testUnsupportedButEnabled() throws {
    let validator = MessageEncodingHeaderValidator(encoding:
  ...
  func testNoCompressionWhenEnabled() throws {
    let validator = MessageEncodingHeaderValidator(encoding:
      .enabled(.init(enabledAlgorithms: CompressionAlgorithm.all, decompressionLimit: .absolute(10)))
    )
