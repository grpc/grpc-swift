...
// This class would be generated by the `protoc` plugin.
final class EchoCallHandlerProvider: CallHandlerProvider {
  var serviceName: String { return "echo.Echo" }

  let provider: EchoGRPCProvider

  init(provider: EchoGRPCProvider) {
...
class EchoServerTests: BasicEchoTestCase {
  ...
  override func setUp() {
    ...
    server = try! GRPCServer.start(
      hostname: "localhost", port: 5050, eventLoopGroup: eventLoopGroup, serviceProviders: [EchoCallHandlerProvider(provider: EchoGRPCProvider())])
      .wait()
  }
