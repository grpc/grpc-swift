...
extension Echo_EchoProvider_NIO {
  ...
  internal func handleMethod(_ methodName: String, request: HTTPRequestHead, serverHandler: GRPCChannelHandler, channel: Channel, errorHandler: ((Error) -> Void)? = nil) -> GRPCCallHandler? {
    switch methodName {
    case "Get":
      return UnaryCallHandler(channel: channel, request: request, errorHandler: errorHandler) { context in
        return { request in
    ...
    case "Expand":
      return ServerStreamingCallHandler(channel: channel, request: request, errorHandler: errorHandler) { context in
        return { request in
    ...
    case "Collect":
      return ClientStreamingCallHandler(channel: channel, request: request, errorHandler: errorHandler) { context in
        return self.collect(context: context)
      }

    case "Update":
      return BidirectionalStreamingCallHandler(channel: channel, request: request, errorHandler: errorHandler) { context in
