...
extension GRPCTLSVersion {
    init?(_ metadata: NWProtocolTLS.Metadata) {
        if #available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 7.0, *) {
            let nwTLSVersion = sec_protocol_metadata_get_negotiated_tls_protocol_version(metadata.securityProtocolMetadata)
            switch nwTLSVersion {
            case .TLSv10:
                self = .tlsv1
            case .TLSv11:
            ...
            let sslVersion = sec_protocol_metadata_get_negotiated_protocol_version(metadata.securityProtocolMetadata)
            switch sslVersion {
...
extension Channel {
    ...
    func getTLSVersionSync(file: StaticString = #fileID, line: UInt = #line) throws -> GRPCTLSVersion? {
        #if canImport(Network)
        if #available(macOS 10.14, iOS 12.0, tvOS 12.0, watchOS 6.0, *) {
            do {
                ...
                // if it is not a NIOTS channel then we will try NIOSSL if we can import it
            }
        }
        #endif
        #if canImport(NIOSSL)
