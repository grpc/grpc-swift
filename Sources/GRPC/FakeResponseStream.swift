...
public class FakeResponseStream<Request: GRPCPayload, Response: GRPCPayload> {
  ...
  internal func _sendResponsePart(_ part: _GRPCClientResponsePart<Response>) {
    self.send(.responsePart(part))
  }

  internal func _sendError(_ error: Error) {
    self.send(.error(error))
  }

  private func send(_ part: StreamEvent) {
    switch self.filter(part) {
    case .include:
      self.writeOrBuffer(part)

    case .includeAndPrepend(let extraPart):
  ...
  /// Filter out any response parts that we're not interested in. This stops the user from sending
  /// parts out of order that would otherwise be a protocol error.
  private func filter(_ part: StreamEvent) -> FilterResult {
    switch (part, self.sendState) {
    ...
    case (.responsePart(.message), .idle):
      ...
      // behalf.
      self.sendState = .sending
      return .includeAndPrepend(.responsePart(.initialMetadata([:])))
  ...
  private enum FilterResult {
    ...
    /// Include the part, but prepend the given part.
    case includeAndPrepend(_ part: StreamEvent)

    /// Exclude the part.
    case exclude
  }

  private func writeOrBuffer(_ part: StreamEvent) {
    switch self.activeState {
    case .inactive:
      self.responseBuffer.append(part)
...
public class FakeUnaryResponse<Request: GRPCPayload, Response: GRPCPayload>: FakeResponseStream<Request, Response> {
  ...
  public func sendMessage(
    ...
    self._sendResponsePart(.initialMetadata(initialMetadata))
    self._sendResponsePart(.message(.init(response, compressed: false)))
    self._sendResponsePart(.trailingMetadata(trailingMetadata))
    self._sendResponsePart(.status(status))
  }
  ...
  ///   - trailingMetadata: The trailing metadata to send. By default the metadata will be empty.
  public func sendError(_ error: Error, trailingMetadata: HPACKHeaders = [:]) {
    self._sendResponsePart(.trailingMetadata(trailingMetadata))
...
public class FakeStreamingResponse<Request: GRPCPayload, Response: GRPCPayload>: FakeResponseStream<Request, Response> {
  ...
  public func sendInitialMetadata(_ metadata: HPACKHeaders) {
    self._sendResponsePart(.initialMetadata(metadata))
  }

  /// Send a response message to the client.
  ///
  /// - Parameter response: The response to send.
  public func sendMessage(_ response: Response) {
    self._sendResponsePart(.message(.init(response, compressed: false)))
  }
  ...
  public func sendEnd(status: GRPCStatus = .ok, trailingMetadata: HPACKHeaders = [:]) {
    self._sendResponsePart(.trailingMetadata(trailingMetadata))
    self._sendResponsePart(.status(status))
  }
