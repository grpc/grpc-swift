...
the build process with custom build tools. Learn how to use the SwiftGRPCPlugin plugin for the
...
> Note: imports on your `.proto` files will have to include the full path from the target source to the `.proto` file you wish to import.
...
let package = Package(
  ...
  dependencies: [
    ...
    .package(url: "https://github.com/grpc/grpc-swift", from: "1.9.1"),
    ...
  ],
  targets: [
    ...
    .executableTarget(
        name: "YourTarget",
        plugins: [
            .plugin(name: "SwiftGRPCPlugin", package: "grpc-swift")
        ]
    ),
    ...
)
...
configure the plugin to invoke the `protoc` compiler. This is done by adding a `swift-grpc-config.json`
...
> Note: paths to your `.proto` files will have to include the full path from the target source to the `.proto` file location. 
