...
/// Makes a server for gRPC interopability testing.
///
/// - Parameters:
...
///     certificate and CA as set out in the interopability test specification. The common name
...
public func makeInteropabilityTestServer(
  ...
  if useTLS {
    ...
    let caCert = InteropabilityTestCredentials.caCertificate
    let serverCert = InteropabilityTestCredentials.server1Certificate
    let serverKey = InteropabilityTestCredentials.server1Key
