...
/// Contains credentials used for the gRPC interopability tests.
///
...
public struct InteropabilityTestCredentials {
  private init() { }

  /// Self signed gRPC interopability test CA certificate.
  public static let caCertificate = try! NIOSSLCertificate(
    buffer: Array(caCertificatePem.utf8CString),
    format: .pem)

  /// gRPC interopability test server certificate.
  ///
  ...
  /// gRPC interopability test server private key.
  ///
  /// Note: the specification refers to the cerificate and key as "server1", this name is carried
  /// across here.
