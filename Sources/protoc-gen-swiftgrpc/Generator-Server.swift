...
extension Generator {
  ...
  private func printServerProtocol() {
    ...
    if options.generateNIOImplementation {
      ...
      println("\(access) func handleMethod(_ methodName: String, request: HTTPRequestHead, serverHandler: GRPCChannelHandler, channel: Channel, errorHandler: ((Error) -> Void)? = nil) -> GRPCCallHandler? {")
      indent()
      ...
      for method in service.methods {
        ...
        println("return \(callHandlerType)(channel: channel, request: request, errorHandler: errorHandler) { context in")
        indent()
        switch streamingType(method) {
